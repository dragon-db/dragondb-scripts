...
# DragonCP - Manual Sync Script by Dragon DB v2.5 External
...
transfer_folder() {
    ...
    echo "Initiating transfer from server..."
    echo "Syncing from: $source_path/"
    echo "Syncing to: $full_destination/"
    echo ">CLIENT-REQUEST<============================="
    
    # Debug logging for arguments being sent
    echo "DEBUG: Sending arguments to server for transfer_folder:"
    ...
    # Execute server-side script with required parameters
    # Using proper argument escaping for SSH with printf
    ssh "$DEBIAN_USER@$DEBIAN_IP" "bash $SERVER_SCRIPT_PATH transfer_folder $(printf '%q' "$source_path") $(printf '%q' "$destination_path") $(printf '%q' "$CLIENT_IP") $(printf '%q' "$CLIENT_USER") $(printf '%q' "$CLIENT_PORT")"
...
sync_single_episode() {
    ...
    echo "Initiating episode transfer from server..."
    ...
    # Debug logging for arguments being sent
    echo "DEBUG: Sending arguments to server for sync_episode:"
    echo "DEBUG: source_path: '$source_path'"
    echo "DEBUG: destination_path: '$destination_path'"
    ...
    # Execute server-side script with required parameters
    # Using proper argument escaping for SSH with printf
    ssh "$DEBIAN_USER@$DEBIAN_IP" "bash $SERVER_SCRIPT_PATH sync_episode $(printf '%q' "$source_path") $(printf '%q' "$destination_path") $(printf '%q' "$source_file") $(printf '%q' "$CLIENT_IP") $(printf '%q' "$CLIENT_USER") $(printf '%q' "$CLIENT_PORT")"
